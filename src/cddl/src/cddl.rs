        ...
        .map(|grpent| match grpent.as_rule() {
            parser::Rule::grpent_memberkey => {
                ...
                if third.is_none() && second.is_none() {
                    GroupEntry::MemberKey(Occurence::Once, create_type(first), None)
...
fn transform_cddl(pairs: Pairs<parser::Rule>) -> Ruleset {
...
pub fn parse_to_pairs(
    content: &str,
) -> Result<Pairs<parser::Rule>, pest::error::Error<parser::Rule>> {
    parser::CDDLParser::parse(parser::Rule::cddl, content)
}

pub fn parse(content: &str) -> Result<Ruleset, Error> {
    Ok(transform_cddl(parse_to_pairs(content)?))
}
